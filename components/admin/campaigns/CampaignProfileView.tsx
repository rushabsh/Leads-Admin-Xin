        ...
        <>
          ...
          <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <span className="text-xs font-semibold text-slate-500 uppercase block">Total Budget</span>
              <span className="text-xl font-extrabold text-slate-900 mt-1 block">${selectedCampaign.budget.toLocaleString()}</span>
              <span className="text-xs text-slate-400 mt-1 block">
                Expected leads target: {selectedCampaign.expectedLeadTarget || 0}
              </span>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <span className="text-xs font-semibold text-slate-500 uppercase block">Total Spend</span>
              <span className="text-xl font-extrabold text-slate-900 mt-1 block">${metrics.actualSpend.toLocaleString()}</span>
              <span className="text-xs text-emerald-600 font-semibold mt-1 block">
                Remaining: ${(selectedCampaign.budget - metrics.actualSpend).toLocaleString()}
              </span>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <span className="text-xs font-semibold text-slate-500 uppercase block">Leads Ingested</span>
              <span className="text-xl font-extrabold text-slate-900 mt-1 block">{metrics.totalLeads}</span>
              <span className="text-xs text-blue-600 font-semibold mt-1 block">
                Qualified: {metrics.qualifiedLeads} ({metrics.conversion}%)
              </span>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <span className="text-xs font-semibold text-slate-500 uppercase block">Cost Per Lead (CPL)</span>
              <span className="text-xl font-extrabold text-slate-900 mt-1 block">${metrics.costPerLead}</span>
              <span className="text-xs text-slate-400 mt-1 block">
          ...
          <div className="min-h-80">
              ...
              <div className="space-y-6">
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500">Leads Over Time</h4>
                    <div className="h-56">
                  ...
                  {/* 2. Leads by Vendor */}
                  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500">Leads by Vendor</h4>
                    <div className="h-56">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 border-b pb-2 border-slate-100">
                    Recent Campaign Activity
                  </h4>
                  <div className="relative border-l border-slate-200 pl-4 ml-2 text-xs space-y-4">
                    <div className="relative">
                      <span className="absolute -left-6 top-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-emerald-600 text-white">
                        <Check className="h-2 w-2" />
                      </span>
                      <span className="font-semibold text-slate-900">Campaign Activated</span>
                      <p className="text-slate-500 text-xs mt-0.5">
                        Initial campaign launch with budget ${selectedCampaign.budget.toLocaleString()}
                      </p>
                    </div>
                    <div className="relative">
                      <span className="absolute -left-6 top-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-blue-600 text-white">
                ...
                <table className="w-full border-collapse text-left text-xs">
                  <thead>
                    <tr className="border-b border-slate-200 bg-slate-50 text-slate-500 uppercase tracking-wider text-xs font-semibold">
                      ...
                      <th className="p-4">Leads Submitted</th>
                      <th className="p-4">Qualified Leads</th>
                      <th className="p-4">Conversion %</th>
                      <th className="p-4">Cost Per Lead (Est)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {metrics.vendorMetrics.length === 0 ? (
                      ...
                      metrics.vendorMetrics.map((v: any) => (
                        <tr key={v.name} className="hover:bg-slate-50/60 transition-colors">
                          ...
                          <td className="p-4 text-slate-900">{v.submitted}</td>
                          <td className="p-4 text-emerald-600 font-bold">{v.qualified}</td>
                          <td className="p-4 font-semibold text-blue-600">{v.conversion}%</td>
                          <td className="p-4 text-slate-900">${selectedCampaign.costPerLeadTarget || 40}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
            ...
            {activeDetailTab === 'lawfirms' && (
              <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <table className="w-full border-collapse text-left text-xs">
                  <thead>
                    <tr className="border-b border-slate-200 bg-slate-50 text-slate-500 uppercase tracking-wider text-xs font-semibold">
                      <th className="p-4">Law Firm Name</th>
                      <th className="p-4">Assigned Leads</th>
                      <th className="p-4">Accepted Leads</th>
                      <th className="p-4">Rejected Leads</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {metrics.lawFirmMetrics.length === 0 ? (
                      ...
                      metrics.lawFirmMetrics.map((f: any) => (
                        <tr key={f.name} className="hover:bg-slate-50/60 transition-colors">
                          ...
                          <td className="p-4 text-slate-900">{f.assigned}</td>
                          <td className="p-4 text-emerald-600 font-bold">{f.accepted}</td>
                          <td className="p-4 text-rose-600 font-bold">{f.rejected}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
            ...
            {activeDetailTab === 'budget' && (
              <div className="grid gap-6 md:grid-cols-2 text-xs">
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
                  <h4 className="text-sm font-bold uppercase tracking-wider text-slate-400 border-b pb-2 border-slate-100">
                    Budget Summary
                  </h4>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-slate-500">Media Budget Limit</span>
                      <span className="font-semibold text-slate-900">${selectedCampaign.budget.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Amount Spent (to Date)</span>
                      <span className="font-semibold text-slate-900">${metrics.actualSpend.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-slate-500">Target CPL (Cost per Lead)</span>
                      <span className="font-semibold text-slate-900">${selectedCampaign.costPerLeadTarget || 40}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Actual CPL</span>
                      <span className="font-semibold text-slate-900">${metrics.costPerLead}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Estimated Revenue</span>
                      <span className="font-semibold text-emerald-600">${metrics.revenue.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Estimated ROI</span>
                      <span
                        className={`font-semibold ${metrics.roi >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                      >
                        {metrics.roi}%
                      </span>
                    </div>
            ...
            {activeDetailTab === 'activity' && (
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 border-b pb-2 border-slate-100">
                  Audit log
                </h4>
                <div className="space-y-3 font-mono text-xs leading-relaxed">
                  <div className="text-slate-600 border-b border-dashed pb-1.5 border-slate-100">
                    <span className="text-slate-400">[2026-07-08 10:15]</span> Campaign{' '}
                    <span className="text-blue-600 font-bold">"{selectedCampaign.name}"</span> created by Admin.
                  ...
                  {selectedCampaign.lawFirm && (
                    <div className="text-slate-600 border-b border-dashed pb-1.5 border-slate-100">
                      ...
                      <span className="text-amber-600 font-bold">"{selectedCampaign.lawFirm.name}"</span> linked as
