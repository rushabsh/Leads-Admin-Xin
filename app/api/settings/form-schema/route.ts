...
const DEFAULT_FORM_FIELDS: FormFieldDefinition[] = [
  ...
  { id: '3', name: 'type', label: 'Tort Category', type: 'select', section: 'leadInfo', required: true, active: true, options: ['Depo-Provera', 'PFAS', 'Rideshare', 'Roblox', 'LA County JDC Sexual Abuse', 'Roundup', 'Storm', 'Talcum', 'Wildfire', "Women's Prisoner Abuse", 'Camp Lejeune', 'NEC Baby Formula', 'Hair Straightener', 'Boy Scouts', 'Mesothelioma', 'AFFF Firefighting Foam', 'Toxic Water Contamination', 'Medical Malpractice', 'Personal Injury', 'Mass Tort - General', 'Other'] },
  { id: '4', name: 'status', label: 'Initial Status', type: 'select', section: 'leadInfo', required: true, active: true, options: ['New', 'In Progress', 'Sent'] },
  ...
  { id: '6', name: 'substatus', label: 'TCPA Substatus', type: 'select', section: 'leadInfo', required: false, active: true, options: ['None', 'No TCPA', 'Redo TCPA', 'TCPA OK'] },
  ...
  { id: '20', name: 'gender', label: 'Gender', type: 'select', section: 'contactInfo', required: false, active: true, options: ['Male', 'Female', 'Other', 'Prefer Not to Say'] },
  ...
  // ==========================================
  { id: '34', name: 'incidentType', label: 'Which Incident Occurred', type: 'select', section: 'diagnosis', required: false, active: true, options: ['Oral Vaginal/anal – Rape', 'Vaginal/anal – Penetration', 'Digital penetration', 'Grooming / Sexual Exploitation', 'Physical Abuse', 'Other'] },
  { id: '35', name: 'diagnosis', label: 'Medical Diagnosis', type: 'select', section: 'diagnosis', required: true, active: true, options: ['Sexual Dysfunction', 'PTSD (Post-Traumatic Stress Disorder)', 'Anxiety', 'Depression', 'Non-Hodgkin Lymphoma', 'Renal Carcinoma / Kidney Cancer', 'Leukemia / Blood Cancer', 'Ovarian Cancer', 'Parkinson\'s Disease', 'Prostate Cancer', 'Bladder Cancer', 'Liver Cancer / Damage', 'Multiple Myeloma', 'Necrotizing Enterocolitis (NEC)', 'Uterine Cancer', 'Respiratory Illness / Lung Cancer', 'Trauma / Physical Injury', 'Sexual Abuse / Trauma', 'Property / Environmental Loss', 'Other Medical Condition'] },
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (vendorId && campaignId) {
      if (overrides[`${vendorId}_${campaignId}`]) {
        fields = overrides[`${vendorId}_${campaignId}`];
      } else {
        const matchingKey = Object.keys(overrides).find(
          (k) => k.toLowerCase() === `${vendorId}_${campaignId}`.toLowerCase()
        );
        if (matchingKey && overrides[matchingKey]) {
          fields = overrides[matchingKey];
        } else if (overrides[vendorId]) {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (vendorId && campaignId) {
      ...
      // When saving Globally (all vendors & campaigns), synchronize field required & active states
      // across all existing overrides so the global change takes effect in all vendors and campaigns
      for (const key of Object.keys(overrides)) {
        if (Array.isArray(overrides[key])) {
          overrides[key] = overrides[key].map((f) => {
            const match = newFields.find((nf) => nf.id === f.id || nf.name === f.name);
            return match ? { ...f, required: match.required, active: match.active, label: match.label } : f;
          });
        }
      }
    }
