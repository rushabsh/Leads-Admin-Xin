        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${campaign.status === 'ACTIVE'
                  ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                  : campaign.status === 'PAUSED'
                    ? 'bg-amber-50 text-amber-700 border border-amber-200'
                    : 'bg-slate-100 text-slate-600 border border-slate-200'
                }`}>
                <span className={`h-1.5 w-1.5 rounded-full ${campaign.status === 'ACTIVE' ? 'bg-emerald-600' : campaign.status === 'PAUSED' ? 'bg-amber-600' : 'bg-slate-500'
                  }`} />
                {campaign.status}
              </span>
            </div>
            ...
            <table className="w-full border-collapse text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedLeads.map((l: any) => {
                  ...
                  const leadTargetId = l.id || l.leadId;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-bold ${(l.leadScore || 70) >= 80
                            ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                            : (l.leadScore || 70) >= 60
                      ...
                      <td className="p-4">
                        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold ${['QUALIFIED', 'SIGNED_RETAINER'].includes(l.status)
                            ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                            : l.status === 'CONTACTED'
                              ? 'bg-indigo-50 text-indigo-700 border border-indigo-200'
                              : l.status === 'REJECTED'
                          ...
                          <span className={`h-1.5 w-1.5 rounded-full ${['QUALIFIED', 'SIGNED_RETAINER'].includes(l.status)
                              ? 'bg-emerald-600'
                              : l.status === 'CONTACTED'
                                ? 'bg-indigo-600'
                                : l.status === 'REJECTED'
